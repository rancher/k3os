    ...
    case $FROM in
        ftp*|http*|tftp*)
            ...
            do
                ...
                n=$((n+1))
                printf "Failed to download, retry attempt %s out of %s\\n" "${n}" "${attempts}"
                sleep 2
            done
            ;;
        *)
...
cat << EOF
Usage: ${PROG} [--force-efi] [--debug] [--tty TTY] [--poweroff] [--takeover] [--no-format] [--config https://.../config.yaml] DEVICE ISO_URL

Example: ${PROG} /dev/vda https://github.com/rancher/k3os/releases/download/v0.8.0/k3os.iso
...
do_format()
{
    ...
    if [ ! -e "${PREFIX}${STATE_NUM}" ]; then
        printf "Failed to find %s%s or %s%s to format\\n" "${PREFIX}" "${STATE_NUM}" "${DEVICE}" "${STATE_NUM}"
        exit 1
    fi
...
do_copy()
{
    ...
    if [[ -n "$STATE_NUM" ]]; then
        printf "%s %s\\n" "${DEVICE}" "${STATE_NUM}" > "$TARGET/k3os/system/growpart"
    fi
...
EOF
    ...
    grub-install ${GRUB_TARGET:+"$GRUB_TARGET"} --boot-directory="${TARGET}/boot" --removable "${DEVICE}"
}
...
get_iso()
{
    ...
    if [[ -z "${ISO_DEVICE}" ]]; then
        printf "#### There is no k3os ISO device\\n"
        return 1
    fi
}
...
setup_style()
{
    if [[ "$K3OS_INSTALL_FORCE_EFI" = "true" ]] || [[ -e "/sys/firmware/efi" ]]; then
        ...
        if [[ ! -e /sys/firmware/efi ]]; then
            printf "WARNING: installing EFI on to a system that does not support EFI\\n"
        fi
    else
        PARTTABLE="msdos"
        BOOTFLAG="boot"
...
validate_device()
{
    ...
    if [[ ! -b "${DEVICE}" ]]; then
        printf "You should use an available device. Device %s does not exist.\\n" "${DEVICE}"
        exit 1
    fi
}
...
main(){
    ...
    if [[ "$K3OS_INSTALL_POWER_OFF" = true ]] || grep -q 'k3os.install.power_off=true' /proc/cmdline; then
        ...
        printf " * Rebooting system in 5 seconds (CTRL+C to cancel)\\n"
        sleep 5
        reboot -f
    fi
}
