...
func TestGetVersion(t *testing.T) {
			...
			if info.IsDir() && filepath.Dir(path) == "testdata" {
				t.Run(filepath.Base(path), func(t *testing.T) {
					...
					if strings.HasPrefix(path, "error") {
						...
						t.Log(err)
					} else if strings.HasPrefix(path, "noerr") {
						if err != nil {
							t.Fatal(err)
						}
						t.Logf("previous=%s, current=%s, running=%s", ver.Previous, ver.Current, ver.Runtime)
						if strings.HasSuffix(path, "missing-previous") && ver.Previous != "" {
							t.Fatal("not missing previous")
						} else if strings.HasSuffix(path, "with-previous") && ver.Previous == "" {
					...
					if ver.Runtime != "" {
						t.Fatalf("expecting empty runtime version: %s", ver.Runtime)
					}
				})
			}
			return nil
	...
	t.Run("WithoutPrefix", func(t *testing.T) {
		filepath.Walk("testdata", func(path string, info os.FileInfo, err error) error {
			if info.IsDir() && filepath.Dir(path) == "testdata" {
				rootDirectory = filepath.Join(path, DefaultRootDir)
				...
				t.Run(base, func(t *testing.T) {
					...
					if ver.Runtime == "" {
						t.Fatalf("missing runtime version")
					}
				})
			}
			return nil
