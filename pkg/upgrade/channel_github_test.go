...
func TestChannel(t *testing.T) {
	spec.Run(t, "https://github.com/rancher/k3os/releases/latest", func(t *testing.T, when spec.G, it spec.S) {
		var (
			channel Channel
			err     error
		)
		it.Before(func() {
			...
			channel, err = NewChannel("https://github.com/rancher/k3os/releases/latest")
			if err != nil {
				t.Fatal(err)
			}
		})
