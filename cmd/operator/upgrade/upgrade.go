...
var Command = cli.Command{
	...
	Flags: []cli.Flag{
		cli.GenericFlag{
			...
			Value: &Channel{
				uri: "github-releases://rancher/k3os",
			},
		},
		cli.StringFlag{
			Name:   "version",
...
func Run(c *cli.Context) {
	// establish the lock
	lockFile, err := os.OpenFile(c.String("lock-file"), os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		logrus.Fatal(err)
	}
...
func downloadRootfs(dir string, rel *upgrade.Release) error {
	...
	return util.Stream(rootfsAsset.URL, func(body io.Reader) error {
		...
		for {
			...
			if strings.HasPrefix(header.Name, rel.Name+`/k3os/system`) {
